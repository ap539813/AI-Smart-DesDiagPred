...
def descriptive_analysis(kpi_column, groupby_columns=None):
    df = preprocess_data("PredDescDiag_experimental/Warehouse_and_Retail_Sales_enhanced.csv", kpi_column)
    threshold = df[kpi_column].mean()
    
    df["Above_Threshold"] = df[kpi_column] > threshold
    df["Rate_Change"] = df[kpi_column].pct_change()
    df["Rate_Above_Threshold"] = df["Rate_Change"] > 0.05  
    
    summary = {
        "latest_period_above_threshold": df[df["Above_Threshold"]].iloc[-1].to_dict(),
        "current_vs_last_year": df[kpi_column].pct_change(12).iloc[-1],
        "current_vs_last_month": df[kpi_column].pct_change().iloc[-1],
    }
    return summary
...
# Diagnostic Analysis
def diagnostic_analysis(kpi_column, groupby_columns=None):
    df = preprocess_data("PredDescDiag_experimental/Warehouse_and_Retail_Sales_enhanced.csv", kpi_column)
    threshold = df[kpi_column].mean()
    latest_anomaly = df[df[kpi_column] > threshold].iloc[-1]
    
    feature_columns = [col for col in df.columns if col not in ["DATE", kpi_column]]
    ...
    results = {"Anomaly Period": latest_anomaly.to_dict()}
    for i, feature_idx in enumerate(top_features):
        feature_name = feature_columns[feature_idx]
        top_values = df[feature_name].value_counts().head(3).index.tolist()
        top_kpi_values = {val: df[df[feature_name] == val][kpi_column].mean() for val in top_values}
        results[f"Top Contributing Factor {i+1}"] = {"Feature": feature_name, "Values": top_kpi_values}
    
    return results

# Forecasting Analysis
def forecasting_analysis(kpi_column, forecast_periods=12):
    df = preprocess_data("PredDescDiag_experimental/Warehouse_and_Retail_Sales_enhanced.csv", kpi_column)
    df.set_index("DATE", inplace=True)
